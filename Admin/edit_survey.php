...
// Function to handle the update of a survey
function updateSurvey($conn, $survey_id, $title, $description, $reward, $questions) {
    ...
    foreach ($questions as $questionData) {
        ...
        if ($questionData['type'] === 'multiple choice' && isset($questionData['options'])) {
            ...
            foreach ($questionData['options'] as $optionData) {
                $options[] = $optionData['option_text'];
            }
            $questionJson['options'] = $options;
        }
        
        // Add options for rating questions (1-5 scale)
        if ($questionData['type'] === 'rating') {
            $questionJson['options'] = ["1", "2", "3", "4", "5"];
        }
        
        // Add correct answer if provided
        if (!empty($questionData['correct_answer'])) {
            $questionJson['correct_answer'] = $questionData['correct_answer'];
        }
        
        $questionsJson[] = $questionJson;
    }
    
    // Update the survey in the surveys table with JSON questions
    $updateSurveyQuery = "UPDATE surveys SET title = :title, description = :description, questions = :questions, reward = :reward, updated_at = NOW() WHERE id = :id";
    $stmt = $conn->prepare($updateSurveyQuery);
    $stmt->bindParam(':title', $title);
    $stmt->bindParam(':description', $description);
    $stmt->bindParam(':questions', json_encode($questionsJson));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($reward) || empty($questions)) {
        ...
        if (updateSurvey($conn, $survey_id, $title, $description, $reward, $questions)) {
            header("Location: manage_surveys.php?message=Survey+updated+successfully");
            exit;
        } else {
            $error_message = "Failed to update survey.";
        }
    }
}
...
if (!empty($survey['questions'])) {
    $questionsData = json_decode($survey['questions'], true);
    if ($questionsData) {
        foreach ($questionsData as $index => $questionData) {
            ...
            if ($question['type'] === 'multiple choice' && isset($questionData['options'])) {
                foreach ($questionData['options'] as $optionText) {
                    $question['options'][] = [
                        'option_text' => $optionText,
                        'is_correct' => 0 // We don't store which option is correct in the JSON format
                    ];
                }
            }
...
<body class="bg-gray-50 min-h-screen">
        ...
        <div class="question-card bg-white border border-gray-200 rounded-lg p-4 fade-in">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="questions[0][type]" onchange="handleQuestionTypeChange(this)"
                         class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
            ...
            <div class="rating-container mb-4 hidden">
                <label class="block text-sm font-medium text-gray-700 mb-2">Rating Scale (1-5)</label>
                <div class="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                    <span class="text-sm text-gray-600">1 (Poor)</span>
                    <div class="flex space-x-2">
                        <span class="w-8 h-8 bg-red-200 rounded-full flex items-center justify-center text-sm font-medium">1</span>
                        <span class="w-8 h-8 bg-orange-200 rounded-full flex items-center justify-center text-sm font-medium">2</span>
                        <span class="w-8 h-8 bg-yellow-200 rounded-full flex items-center justify-center text-sm font-medium">3</span>
                        <span class="w-8 h-8 bg-blue-200 rounded-full flex items-center justify-center text-sm font-medium">4</span>
                        <span class="w-8 h-8 bg-green-200 rounded-full flex items-center justify-center text-sm font-medium">5</span>
                    </div>
                    <span class="text-sm text-gray-600">5 (Excellent)</span>
                </div>
                <p class="text-xs text-gray-500 mt-2">Users will select a rating from 1 to 5 using radio buttons</p>
            </div>
    ...
    <script>
        ...
        function handleQuestionTypeChange(selectElement) {
            ...
            const correctAnswerField = questionCard.querySelector('input[name*="correct_answer"]');
            
            const selectedType = selectElement.value;
            
            // Get all option inputs in this question
            const optionInputs = optionsContainer.querySelectorAll('input[name*="option_text"]');
            const optionCheckboxes = optionsContainer.querySelectorAll('input[name*="is_correct"]');
            
            if (selectedType === 'multiple choice') {
                optionsContainer.classList.remove('hidden');
                ratingContainer.classList.add('hidden');
                correctAnswerField.placeholder = 'Enter correct answer (optional)';
                
                // Enable and make option inputs required
                ...
                correctAnswerField.placeholder = 'Expected rating (1-5, optional)';
                
                // Disable and remove required from option inputs
                optionInputs.forEach(input => {
                    input.disabled = true;
                ...
                optionsContainer.classList.add('hidden');
                ratingContainer.classList.add('hidden');
                correctAnswerField.placeholder = 'Expected answer (optional)';
        ...
        function addQuestion(data = null) {
            ...
            if (optionsContainer) {
                ...
                if (data && data.type === 'multiple choice' && data.options && data.options.length > 0) {
                    data.options.forEach((option, index) => {
                        const optionDiv = document.createElement('div');
                        optionDiv.className = 'flex items-center space-x-2';
                        optionDiv.innerHTML = `
                            <input type="text" name="questions[${questionCount}][options][${index}][option_text]"
                                class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                    ...
                    optionDiv.className = 'flex items-center space-x-2';
                    optionDiv.innerHTML = `
                        <input type="text" name="questions[${questionCount}][options][0][option_text]"
                            class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                            placeholder="Option 1">
            ...
            // Set up the question type change handler
            const typeSelect = newQuestion.querySelector('select[name*="[type]"]');
            if (typeSelect) {
                typeSelect.setAttribute('onchange', 'handleQuestionTypeChange(this)');
            }

            questionsDiv.appendChild(newQuestion);
            questionCount++;
            
            // Initialize the question type (after adding to DOM)
            if (typeSelect) {
